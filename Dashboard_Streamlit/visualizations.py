...
visualizations.py
=================
Modul fungsi pembuatan chart Plotly untuk Recruitment Analytics Dashboard.
...
def fig_bq6(df_glints: pd.DataFrame, top_n: int = 15) -> tuple[go.Figure, dict]:
    ...
    skill_counts = skill_counts.sort_values("Persen", ascending=True)
...
def fig_bq7(df_cv: pd.DataFrame) -> tuple[go.Figure, dict]:
    ...
    fig.update_layout(
        ...
        template=TEMPLATE,
        height=480,
        yaxis_title="Rata-rata Jumlah Skill",
...
def fig_bq8(df_glints: pd.DataFrame) -> tuple[go.Figure, dict]:
    ...
    color_map = {}
    color_map[penuh_waktu_labels[0]] = COLOR_WFO
    ...
    fig = px.bar(
        ...
        labels={
            "Persen": "Persentase (%)",
            GLINTS_ROLE_COL: "Kategori Peran",
            GLINTS_WORK_TIME_COL: "Tipe Waktu Kerja"
        },
        category_orders={
            GLINTS_ROLE_COL: sorted_roles,
            GLINTS_WORK_TIME_COL: ordered_time_types
        },
        color_discrete_map=color_map
    )
    fig.add_vline(
        x=70, line_dash="dash", line_color=COLOR_THRESHOLD,
...
def fig_bq9(df_job: pd.DataFrame) -> tuple[go.Figure, go.Figure, dict]:
    """
    BQ 9: Persentase lowongan dengan batasan gender/usia, top posisi pembatas.
...
def fig_bq10(df_glints: pd.DataFrame) -> tuple[go.Figure, go.Figure, dict]:
    ...
    heat_data = pivot.pivot(
        index=GLINTS_ROLE_COL, columns="Sistem Kerja", values="Median Gaji"
    ).fillna(0)
    ...
    fig_heat = px.imshow(
        ...
        title="BQ 10 — Heatmap Median Gaji (Kategori x Sistem Kerja)"
    )
